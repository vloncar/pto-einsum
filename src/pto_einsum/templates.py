...
einsum_config_template = """struct config_einsum {{
    ...
    // NT strided-input matmul (drop Phase A). When the contraction axis is the
    // innermost (contiguous) axis of BOTH inputs and free0/free1 are each a single
    ...
    // transpose round-trip. in_nt selects the read modes: 1 = NT (B DN-transposed), 2 =
    // NN-strided (B ND with a strided K row), 3 = TN (A DN-transposed: free0 contiguous,
    // contraction strided by in0_k_stride; B NN-strided). in0_row_stride / in1_col_stride
    // are the source element strides of free0 (input0) / free1 (input1); in0_k_stride /
    // in1_k_stride are input0 / input1's contraction strides (1 unless that operand is read
    // with a strided contraction). The batch arrays decode the flat batch index into each
    // operand's source base (inplace-axis sizes shared, per-operand source strides). All
    // zero/neutral when not direct-read-eligible.
    ...
    static constexpr unsigned in0_k_stride = {in0_k_stride};
